...
#define blk0(i) (block[i]=be32(((const uint32_t *)buffer)[i]))
...
void sha1_final(SHA1_CTX *ctx, uint8_t digest[20])
{
    ...
    sha1_update(ctx, (uint8_t *)&finalcount, 8);  /* Should cause a sha1_transform() */
    for (i = 0; i < 5; i++) {
        ((uint32_t *)digest)[i] = be32(ctx->state[i]);
    }
}
